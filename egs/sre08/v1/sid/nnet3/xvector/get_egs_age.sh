...
# Apache 2.0
#
# This script dumps training examples (egs) for multiclass age detection.
# These egs consist of a data chunk and zero-based age label.
# Each archive of egs has a different input chunk size.
# We don't mix together different lengths in same archive, because it
# would require to repeatedly run the compilation process within the same
# training job.
#
# TODO: in future, we plan to dump multiple labels per egs with fixed time step to
# compute and backprop error for fixed time-step.
...
min_frames_per_chunk=500
max_frames_per_chunk=3000
...
for f in $data/utt2num_frames $data/feats.scp $data/utt2age; do
...
min_frames_per_iter=$[($num_train_frames*$num_repeats)/$max_archives + 1]
if [ $frames_per_iter -lt $min_frames_per_iter ]; then
  echo "frame-per-iter, $frames_per_iter is less than min value, $min_frames_per_iter "
  echo "and frames_per_iter sets to min value."
  frames_per_iter=$min_frames_per_iter
fi
...
if [ $stage -le 4 ]; then
  ...
  $cmd --max-jobs-run $nj JOB=1:$num_train_archives $dir/log/shuffle.JOB.log \
    nnet3-shuffle-egs --buffer-size=5000 --srand=JOB ark:$dir/egs_temp.JOB.ark \
  ...
  $cmd --max-jobs-run $nj JOB=1:$num_diagnostic_archives $dir/log/train_subset_shuffle.JOB.log \
    nnet3-shuffle-egs --buffer-size=5000 --srand=JOB ark:$dir/train_subset_egs_temp.JOB.ark \
  ...
  $cmd --max-jobs-run $nj JOB=1:$num_diagnostic_archives $dir/log/valid_shuffle.JOB.log \
    nnet3-shuffle-egs --buffer-size=5000 --srand=JOB ark:$dir/valid_egs_temp.JOB.ark \
