...
extractor=               # extractor dir containing final.dubm model used for
                         # i-vector extraction.
feat_suffix=_hires
. ./cmd.sh
. ./path.sh
...
if [ $stage -le 2 ] && [ -f data/${train_set}_sp${feat_suffix}/feats.scp ]; then
  echo "$0: data/${train_set}_sp${feat_suffix}/feats.scp already exists."
...
if [ $stage -le 2 ]; then
  ...
  mfccdir=data/${train_set}_sp${feat_suffix}/data
  ...
  for datadir in ${train_set}_sp ${test_sets}; do
    utils/copy_data_dir.sh data/$datadir data/${datadir}${feat_suffix}
  done

  # do volume-perturbation on the training data prior to extracting hires
  # features; this helps make trained nnets more invariant to test data volume.
  utils/data/perturb_data_dir_volume.sh data/${train_set}_sp${feat_suffix}

  for datadir in ${train_set}_sp ${test_sets}; do
    steps/make_mfcc.sh --nj $nj --mfcc-config $mfcc_config \
      --cmd "$train_cmd" data/${datadir}${feat_suffix}
    steps/compute_cmvn_stats.sh data/${datadir}${feat_suffix}
    utils/fix_data_dir.sh data/${datadir}${feat_suffix}
  done
fi
if [ -e $extractor/final.dubm ]; then
  echo "$0: $extractor/final.dubm already exists and this model used for i-vector"
  echo "    extraction. You can delete the model and rerun the script to re-train "
  echo "    final.dubm model."
else
  if [ $stage -le 3 ]; then
    ...
    utils/data/subset_data_dir.sh --utt-list data/${train_set}/feats.scp \
      data/${train_set}_sp${feat_suffix} $temp_data_root/${train_set}${feat_suffix}

    # note: essentially all the original segments should be in the hires data.
    n1=$(wc -l <data/${train_set}/feats.scp)
    n2=$(wc -l <$temp_data_root/${train_set}${feat_suffix}/feats.scp)
    if [ $n1 != $n1 ]; then
      echo "$0: warning: number of feats $n1 != $n2, if these are very different it could be bad."
      sleep 5
    fi

    echo "$0: training a system on the ${feat_suffix} data for its LDA+MLLT transform, in order to produce the diagonal GMM."
    ...
    steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 7 --mllt-iters "2 4 6" \
       ...
       3000 10000 $temp_data_root/${train_set}${feat_suffix} data/lang \
  ...
  if [ $stage -le 4 ]; then
    ...
    num_utts_total=$(wc -l <data/${train_set}_sp${feat_suffix}/utt2spk)
    num_utts=$[$num_utts_total/4]
    utils/data/subset_data_dir.sh data/${train_set}_sp${feat_suffix} \
        $num_utts ${temp_data_root}/${train_set}_sp${feat_suffix}_subset
    ...
    steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
      ...
      ${temp_data_root}/${train_set}_sp${feat_suffix}_subset 512 \
  ...
  if [ $stage -le 5 ]; then
    ...
    steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
      data/${train_set}_sp${feat_suffix} exp/nnet3${nnet3_affix}/diag_ubm $extractor || exit 1;
  fi
fi
...
if [ $stage -le 6 ]; then
  ...
  ivectordir=exp/nnet3${nnet3_affix}/ivectors_${train_set}_sp${feat_suffix}
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/${train_set}_sp${feat_suffix} ${temp_data_root}/${train_set}_sp${feat_suffix}_max2

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    ${temp_data_root}/${train_set}_sp${feat_suffix}_max2 \
    $extractor $ivectordir

  # Also extract iVectors for the test data, but in this case we don't need the speed
  # perturbation (sp).
  for data in ${test_sets}; do
    nspk=$(wc -l <data/${data}${feat_suffix}/spk2utt)
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "${nspk}" \
      data/${data}${feat_suffix} $extractor \
      exp/nnet3${nnet3_affix}/ivectors_${data}${feat_suffix}
  done
fi
