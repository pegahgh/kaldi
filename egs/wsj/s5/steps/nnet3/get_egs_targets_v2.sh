...
# get_egs_targets_v2.sh is as get_egs_targets.sh, but it uses frames_per_iter
# instead of samples_per_iter.
# It helps for better convergence and stable training
set -o pipefail
trap "" PIPE
...
avg_frame_per_eg=$(echo $frames_per_eg | tr ',' ' ' | awk '{for (i =0; i <NF; i++) sum+=$i } END {print int(sum/NF)}')
echo avg-frames-per-eg = $avg_frame_per_eg
egs_per_archive=$[$num_frames/($avg_frame_per_eg*$num_archives)] || exit 1;
