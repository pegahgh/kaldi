...
read_input_prob=`perl -e "print $(cat ${common_egs_dir[0]}/info/num_frames)/$tot_frames"`
...
for lang in `seq 1 $[$num_lang-1]`; do
    egs_dir=${common_egs_dir[$lang]}
    new_prob=`perl -e "print $(cat ${common_egs_dir[$lang]}/info/num_frames)/$tot_frames"`
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    for i in `seq 0 $[$num_lang-1]`;do
      ...
      if $separate_init_layer_per_output; then
        rename_io_names="$rename_io_names,input-$i/input"
        if $use_ivector; then
          rename_io_names="$rename_io_names,ivector-$i/ivector"
        fi
      fi
    ...
    if [ $x -gt 0 ]; then
      ...
      if $separate_init_layer_per_output; then
        rename_io_names="$rename_io_names,input-0/input"
        if $use_ivector; then 
          rename_io_names="${rename_io_names},ivector-0/ivector"
        fi
      fi
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      for n in $(seq $this_num_jobs); do
        ...
        for lang in `seq 0 $[$num_lang-1]`; do
          ...
          if $separate_init_layer_per_output; then
            rename_io_names="$rename_io_names,input/input-$lang"
            if $use_ivector; then
              rename_io_names="$rename_io_names,ivector/ivector-$lang"
            fi
          fi
...
if [ $stage -le $num_iters ]; then
  ...
  for lang in `seq 0 $[$num_lang-1]`; do
    ...
    rename_io_names="output/output-$lang"
    if $separate_init_layer_per_output; then
      rename_io_names="$rename_io_names,input/input-$lang,ivector/ivector-$lang"
      if $use_ivector; then
        rename_io_names="$rename_io_names,ivector/ivector-$lang"
      fi
    fi
  ...
  for lang in `seq 0 $[$num_lang-1]`;do
    ...
    if $separate_init_layer_per_output; then
      rename_io_names="$rename_io_names,input-${lang}/input"
      if $use_ivector; then
        rename_io_names="$rename_io_names,ivector-${lang}/ivector"
      fi
    fi
