...
frames_per_eg=150
remove_egs=false
dir=exp/chain/tdnn_fdnnc_lda # Note: _sp will get added to this if $speed_perturb == true.
...
dir=${dir}${affix:+_$affix}${feat_affix:+$feat_affix}$suffix
...
if [ $stage -le 12 ]; then
  ...
  for data_set in $train_set eval2000 rt03 train_dev;do
    ...
    steps/make_dnnc.sh --nj 80 --cmd "$train_cmd" \
      --dnnc-config conf/fdnnc${feat_affix}.conf \
      data/${data_set}_fdnnc${feat_affix} data/${data_set}_fdnnc${feat_affix}/log \
      $fdnnc_dir
  done
fi
...
if [ $stage -le 14 ]; then
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --feat-dir data/${train_set}_fdnnc${feat_affix} \
    --tree-dir $treedir \
...
if [ $stage -le 16 ]; then
  ...
  #for decode_set in eval2000; do
      ...
      steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
          ...
          $graph_dir data/${decode_set}_fdnnc${feat_affix} \
          $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_sw1_tg || exit 1;
      if $has_fisher; then
          steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
            data/lang_sw1_{tg,fsh_fg} data/${decode_set}_fdnnc${feat_affix} \
