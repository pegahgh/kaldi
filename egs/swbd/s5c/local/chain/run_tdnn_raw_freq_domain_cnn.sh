...
get_egs_stage=-10
speed_perturb=true
dir=exp/chain/tdnn_raw_freq_domain_cnn  # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_nj=50
...
if [ $stage -le 13 ]; then
  ...
  preprocess-fft-abs-lognorm-affine-log-layer name=raw0 input=Append(-1,0,1) cos-transform-file=$dir/configs/cos_transform.mat sin-transform-file=$dir/configs/sin_transform.mat num-filters=$num_filters dim=80 half-fft-range=true scale=1.0
  conv-relu-batchnorm-layer name=cnn1 height-in=$num_filters height-out=50 time-offsets=-1,0,1 height-offsets=-2,-1,0,1,2 num-filters-out=32 height-subsample-out=2
