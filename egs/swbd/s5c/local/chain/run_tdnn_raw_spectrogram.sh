#!/bin/bash
# _raw_spectrogram is as _raw_freq_domain, except it trains the filters on log of power
# spectrum (spectrogram features) instead of power spectrum.
...
get_egs_stage=-10
speed_perturb=true
dir=exp/chain/tdnn_raw_spectrogram  # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_nj=50
...
if [ $stage -le 13 ]; then
  ...
  preprocess-fft-abs-log-layer name=raw0 input=Append(-1,0,1) cos-transform-file=$dir/configs/cos_transform.mat sin-transform-file=$dir/configs/sin_transform.mat num-filters=$num_filters dim=80 half-fft-range=true
  relu-batchnorm-layer name=filter1 dim=$num_filters
