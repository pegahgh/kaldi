...
int main(int argc, char *argv[]) {
  try {
    ...
    for (; !feat_reader1.Done(); feat_reader1.Next()) {
      ...
      int32 min_rows = std::min(feat1.NumRows(), feat2.NumRows());
      Vector<BaseFloat> feat1_col(feat1.NumRows()), feat2_col(feat2.NumRows());
      for (MatrixIndexT i = 0; i < feat1.NumCols(); i++){
        feat1_col.CopyColFromMat(feat1.RowRange(0, min_rows), i);
        feat2_col.CopyColFromMat(feat2.RowRange(0, min_rows), i);
