...
/**
  This function applies image perturbation as directed by 'config'
  on eg and generated two NnetIo containing perturbed copies of original eg.
  The example 'eg' is expected to contain a NnetIo member with
  name 'input', representing an image.
*/
void UnsupPerturbImageExample(
    const ImageAugmentationConfig &config,
    NnetExample &orig_eg,
    NnetExample *pertubed_eg) {
  int32 io_size = orig_eg.io.size();
  ...
  for (int32 i = 0; i < io_size; i++) {
    NnetIo io = orig_eg.io[i];
    if (io.name == "input") {
      found_input = true;
      Matrix<BaseFloat> image;
      io.features.GetMatrix(&image);
      for (int32 c = 0; c < num_cp; c++) {
        Matrix<BaseFloat> perturbed_image(image);
        PerturbImage(config, &perturbed_image, &upside_down,
                     &rgb2bw, &rotation_class);
        NnetIo new_io("input", 0, perturbed_image);
        for (std::vector<Index>::iterator indx_it = new_io.indexes.begin();
             indx_it != new_io.indexes.end(); ++indx_it)
          indx_it->n = c;
        pertubed_eg->io.push_back(new_io);
      }
    } else if (io.name == "output") {
      for (int32 c = 0; c < num_cp; c++) {
        for (std::vector<Index>::iterator indx_it = io.indexes.begin();
                 indx_it != io.indexes.end(); ++indx_it)
          indx_it->n = c;
        pertubed_eg->io.push_back(io);
      }
    }
  }
  if (!found_input)
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "parameters).\n"
        "E.g.:\n"
        "  nnet3-egs-augment-image-unsup --horizontal-flip-prob=0.5 --horizontal-shift=0.1\\\n"
    ...
    for (; !example_reader.Done(); example_reader.Next(), num_done++) {
      ...
      NnetExample perturbed_eg;
      //PerturbImageInNnetExample(config, &eg);
      UnsupPerturbImageExample(config, eg, &perturbed_eg);
      example_writer.Write(key, perturbed_eg);
    }
