...
void RenameIoNames(std::string io_names,
...
int main(int argc, char *argv[]) {
  try {
    ...
    if (use_rand_read) {
      ...
      while (not_finished) { 
        int32 input_archive_num = GenDist(input_probs);
        if (!example_readers[input_archive_num]->Done()) {
          // count is normally 1; could be 0, or possibly >1.
          int32 count = GetCount(keep_proportion);
          std::string key = example_readers[input_archive_num]->Key();
      ...
      // read input archives consequently.
